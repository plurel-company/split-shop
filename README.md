...
Laptop harness: `wrangler.local.jsonc` (`split-shop-local`). Production and Workers Builds read `wrangler.jsonc` (`name`: `splitshop`). `.env.production` bakes `NEXT_PUBLIC_SITE_URL` and `NEXT_PUBLIC_PAY_BASE_URL` into the OpenNext bundle. Workers Builds already runs `pnpm run build` then `npx wrangler deploy`. On Workers CI that script runs OpenNext once, then the nested `pnpm run build` stays on `next build`. Local `pnpm build` stays Next.js only. Preview deploy: `npx wrangler versions upload`.
